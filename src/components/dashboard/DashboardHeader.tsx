...
export const DashboardHeader: React.FC<Props> = ({ onOpenMobileSidebar }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm py-2.5 px-4 md:px-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-blue-50 text-blue-900 border border-blue-200 px-3 py-1.5 rounded-xl text-xs font-semibold">
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl border border-slate-200 shadow-xl p-4 space-y-3 z-50 animate-fadeIn">
                ...
                <div className="space-y-2 text-xs">
                  {notifications.map(n => (
                    <div key={n.id} className={`p-2.5 rounded-xl border ${n.unread ? 'bg-blue-50/70 border-blue-200' : 'bg-slate-50 border-slate-200'}`}>
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden xl:block text-xs leading-tight">
                <p className="font-bold text-slate-900 truncate max-w-[130px]">{user?.name}</p>
                <p className="text-[10px] text-slate-500 font-mono capitalize">{user?.role} Role</p>
              </div>
