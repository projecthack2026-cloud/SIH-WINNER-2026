...
export const DistrictFinancialPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchFinancials = async () => {
    ...
    try {
      ...
      setError("Unable to load district financial metrics from database.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AppShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="badge badge-info text-[10px] uppercase font-bold">FINANCIAL ACCOUNTABILITY DESK</span>
              <span className="text-xs text-slate-500 font-mono">District Nodal Treasury</span>
            </div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight mt-0.5">
              Financial Intelligence — {districtName} District
            </h1>
            <p className="text-xs text-slate-500">
              Monitor expenditure, fund utilization, and cost records across {districtName} District projects.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <KpiCard kpi={{ id: '1', title: 'Total Sanctioned', value: totalSanctioned, description: 'District Allocation', color: 'blue' }} icon={DollarSign} />
          <KpiCard kpi={{ id: '2', title: 'Total Expenditure', value: totalSpent, change: 'Disbursed to date', trend: 'up', color: 'emerald' }} icon={TrendingUp} />
          <KpiCard kpi={{ id: '3', title: 'Utilization Rate', value: `${summary?.overall_utilization_rate || 0}%`, change: `${summary?.total_projects || 0} Works Tracked`, trend: 'up', color: 'emerald' }} icon={BarChart3} />
        </div>

        {/* Project Financial Table */}
        <div className="space-y-3">
          <h3 className="font-extrabold text-slate-900 text-base">District Project Financial Vouchers</h3>
          {loading ? (
            <div className="p-6 bg-white rounded-2xl border border-slate-200 text-center text-xs text-slate-500 flex items-center justify-center gap-2">
              ...
              <span>Loading district financial project records...</span>
            </div>
          ) : (
            <ProjectTable
              projects={projects}
