...
export const StateFinancialPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchFinancials = async () => {
    ...
    try {
      ...
      setError("Unable to load state financial metrics from database.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AppShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="badge badge-info text-[10px] uppercase font-bold">STATE FINANCIAL INTELLIGENCE</span>
              <span className="text-xs text-slate-500 font-mono">State Nodal Account (SNA)</span>
            </div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight mt-0.5">
              State Financial Intelligence — {stateName}
            </h1>
            <p className="text-xs text-slate-500">
              State-wide fund disbursement, utilization, and project financial audit trails.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <KpiCard kpi={{ id: '1', title: 'Total Sanctioned Fund', value: totalSanctioned, description: 'PostgreSQL Sanctions Table', color: 'blue' }} icon={DollarSign} />
          <KpiCard kpi={{ id: '2', title: 'Total Expenditure', value: totalSpent, change: `${summary?.overall_utilization_rate || 0}% Utilization Rate`, trend: 'up', color: 'emerald' }} icon={TrendingUp} />
          <KpiCard kpi={{ id: '3', title: 'Tracked Works', value: summary?.total_projects || projects.length, description: 'PostgreSQL Canonical Works', color: 'purple' }} icon={BarChart3} />
        </div>
        ...
        {loading ? (
          <div className="p-6 bg-white rounded-2xl border border-slate-200 text-center text-xs text-slate-500 flex items-center justify-center gap-2">
            ...
            <span>Loading state financial records...</span>
          </div>
        ) : (
          <ProjectTable
            projects={projects}
